...
def process_job(job):
    ...
    try:
        ...
        # --- Step 4: Generate blockchain proof after fingerprint storage ---
        ...
        if proof_result.get("success"):
            ...
            conn.commit()
            logger.info("✅ Proof stored for %s" % submission_id[:8])
        else:
            logger.warning(
                "⚠️ Proof generation failed for %s: %s" %
                (submission_id[:8], proof_result.get("error"))
            )
